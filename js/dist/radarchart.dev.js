...
function RadarChart(id, data, options) {
  ...
  axisGrid.selectAll(".levels").data(d3.range(1, cfg.levels + 1).reverse()).join("circle").attr("class", "gridCircle").attr("r", function (d, i) {
  ...
  axisGrid.selectAll(".axisLabel").data(d3.range(1, cfg.levels + 1).reverse()).join("text").attr("class", "axisLabel").attr("x", 4).attr("y", function (d) {
  ...
  var axis = axisGrid.selectAll(".axis").data(allAxis).join("g").attr("class", "axis"); //Append the lines
  ...
  var radarLine = d3.lineRadial().curve(d3.curveLinearClosed).radius(function (d) {
  ...
  var blobWrapper = g.selectAll(".radarWrapper").data(data).join("g").attr("class", "radarWrapper"); //Append the backgrounds	

  blobWrapper.append("path").attr("class", "radarArea").attr("d", function (d, i) {
    return radarLine(d);
  }).style("fill", function (d, i) {
    return cfg.color(i);
  }).style("fill-opacity", cfg.opacityArea).on('mouseover', function (event, d) {
    //Dim all blobs
  ...
  blobWrapper.selectAll(".radarCircle").data(function (d, i) {
    return d;
  }).join("circle").attr("class", "radarCircle").attr("r", cfg.dotRadius).attr("cx", function (d, i) {
  ...
  var blobCircleWrapper = g.selectAll(".radarCircleWrapper").data(data).join("g").attr("class", "radarCircleWrapper"); //Append a set of invisible circles on top for the mouseover pop-up

  blobCircleWrapper.selectAll(".radarInvisibleCircle").data(function (d, i) {
    return d;
  }).join("circle").attr("class", "radarInvisibleCircle").attr("r", cfg.dotRadius * 1.5).attr("cx", function (d, i) {
    return rScale(d.value) * Math.cos(angleSlice * i - Math.PI / 2);
  }).attr("cy", function (d, i) {
    return rScale(d.value) * Math.sin(angleSlice * i - Math.PI / 2);
  }).style("fill", "none").style("pointer-events", "all").on("mouseover", function (event, d) {
