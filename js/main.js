const pinPath =
  "M12 2C8.13 2 5 5.13 5 9c0 5.25 7 13 7 13s7-7.75 7-13c0-3.87-3.13-7-7-7zm0 9.75c-1.51 0-2.75-1.24-2.75-2.75S10.49 6.25 12 6.25 14.75 7.49 14.75 9 13.51 11.75 12 11.75z";
const svgPath =
  "M 100,20 \
...
// let priceColor1 = "#005F73"
// let priceColor2 = "#0A9396"
// let priceColor3 = "#94D2BD"
let priceColor1 = "#BB3E03";
let priceColor2 = "#CA6702";
let priceColor3 = "#EE9B00";
let socialColor1 = "#BB3E03";
let socialColor2 = "#CA6702";
let socialColor3 = "#EE9B00";
...
// Load data
Promise.all([
  d3.csv("rentcrime.csv", d3.autoType),
  ...
  us = files[1];
  let data2019 = data.filter((d) => d.year == 2019);
  console.log(data);
  ...
  const groupedData = Array.from(
    ...
    ([state, cities]) => {
      const cityData = Array.from(cities, ([cityname, values]) => {
        const cityValues = values.map((d, i) => {
          d.median = d.price / d.square_feet; // Add price per square foot
          d.name = `Rental ${i + 1}`; // Add name to each listing
          return d.median;
        });
        const median = d3.median(cityValues);
        const totalPets =
          d3.sum(values.map((d) => d.pets_allowed)) / values.length;
      ...
      const stateMedian = d3.median(cityData.map((city) => city.median));
      const medFamInc = d3.median(cityData.map((city) => city.medFamInc));
      const totalPets = d3.mean(cityData.map((city) => city.totalPets));
      const avg_crime = d3.mean(cityData.map((city) => city.avg_crime));
      const black = d3.mean(cityData.map((city) => city.black));
      const white = d3.mean(cityData.map((city) => city.white));
      const asian = d3.mean(cityData.map((city) => city.asian));
      const hisp = d3.mean(cityData.map((city) => city.hisp));
      const violent_crime = d3.mean(cityData.map((city) => city.violent_crime));
      const arson_crime = d3.mean(cityData.map((city) => city.arson_crime));
      const assault_crime = d3.mean(cityData.map((city) => city.assault_crime));
      const gta_crime = d3.mean(cityData.map((city) => city.gta_crime));
      const burg_crime = d3.mean(cityData.map((city) => city.burg_crime));
      const larc_crime = d3.mean(cityData.map((city) => city.larc_crime));
      const murder_crime = d3.mean(cityData.map((city) => city.murder_crime));
      const nonviolent_crime = d3.mean(
        cityData.map((city) => city.nonviolent_crime)
      );
      const rape_crime = d3.mean(cityData.map((city) => city.rape_crime));
      const robbery_crime = d3.mean(cityData.map((city) => city.robbery_crime));
      const population = d3.median(cityData.map((city) => city.population));
      const maxCrimeInState = d3.max(
        cityData.map((city) =>
          Math.max(
            city.violent_crime,
            city.assault_crime,
      ...
      const citiesWithMaxCrimeInState = cityData.map((city) => ({
        ...city,
  ...
  const maxCrimeAcrossAllStates = d3.max(
  ...
  function dependency(price_chart, map, racial_breakdown, avg_crime, sum_pet) {
    ...
    function updateWhenListing() {
      let stateData = price_chart.groupedData.find(
        (c) => c.statename === currentState
      );
      let cityData = stateData.cities.find((b) => b.city === currentCity);
      let listing = cityData.listings.find((l) => l.id === currentListing);
      price_chart.updateScales(cityData.listings);
      price_chart.update(cityData.listings);
      racial_breakdown.updateFillLevels(cityData);

      stateData = avg_crime.groupedData.find(
        (c) => c.statename === currentState
      );
      cityData = stateData.cities.find((b) => b.city === currentCity);
      ...
      stateData = sum_pet.groupedData.find((c) => c.statename === currentState);
      cityData = stateData.cities.find((b) => b.city === currentCity);
    ...
    function updateWhenState() {
      let stateData = sortedData || price_chart.groupedData;
      stateData = stateData.find((c) => c.statename === currentState);
      ...
      sortedData = groupedData
        .map((stateData) => ({
          ...
          cities: stateData.cities
            .map((cityData) => ({
              ...cityData,
              listings: [...cityData.listings].sort(
                (a, b) => b[race] - a[race]
              ),
            }))
  ...
  var sum_pet = initializeSumPet(groupedData);
  // Update dependencies
  ...
  d3.select("#dashboard-subtitle")
    .transition()
    .duration(500)
    .style("opacity", 0)
    .on("end", function () {
      d3.select(this)
        .text("Unveiling Apartment Trends and Socio-Economic Realities")
...
function initializeRacialBreakdown() {
  const races = ["White", "Asian", "Hispanic", "Black"];
  const race_index = ["white", "asian", "hisp", "black"];
  const colors = ["#ffdbac", "#e0ac69", "#8d5524", "#3c2e28"]; // Add colors for each race
  var dependencies;
  ...
  const width = $("#racial-breakdown").width();
  const height = $("#racial-breakdown").height();
  const container = d3
    .select("#racial-breakdown")
    .style("display", "flex")
    .style("justify-content", "space-between")
    .style("text-align", "center");

  const containerWidth = width / races.length; // Get the width of each bar
  ...
  function updateTooltip(event, d) {
    var percentage = 0;
    const race = races[d]; // Get the race name from the races array
    if (race_values == null) {
      percentage = 0;
    } else {
    ...
    race_tooltip
      .style("opacity", 1)
      .html(
        `<strong>${race}</strong><br/>Percentage: ${percentage.toFixed(2)}%`
      )
      .style("left", `${event.pageX - 1630}px`) // offset from mouse
      .style("top", `${event.pageY - 40}px`); // offset from mouse
  }
  races.forEach((race, index) => {
    ...
    const bar = svg
      .append("rect")
      .attr("width", containerWidth - 16) // Adjust bar width
    ...
    const barFill = svg
      ...
      .style("fill", colors[index]); // Use different color for each race
    clipPaths.push(barFill.node());

    svg
      .append("text")
      .attr("x", containerWidth / 2)
      .attr("y", containerHeight - 3.2)
      .style("fill", "white")
  ...
  function updateFillLevels(selectedData) {
    ...
    const fillLevels = race_values.map(
      (value) => (containerHeight - 16) * (value / 100)
    );
    clipPaths.forEach((rect, index) => {
      gsap.to(rect, {
        duration: 1,
...
function initializeMap(apartmentData, groupedData, us) {
  ...
  var maxZoom;
  var dependencies;
  const customInterpolator = d3
    .scaleSequential()
    .domain([0, 1])
    .interpolator((t) => d3.interpolateOrRd(0.2 + 0.8 * t));

  // Create a color scale based on the population
  const populationExtent = d3.extent(groupedData, (d) => d.population);
  const colorScale = d3
    .scaleSequential()
    .domain([populationExtent[0], populationExtent[1]])
    .interpolator(customInterpolator.interpolator());
  function updateDependencies(depends) {
    dependencies = depends;
  }
  const projection = d3
  ...
  const svg = d3
    ...
    .on("click", function (event) {
      ...
      const isPath = clickedElement.tagName.toLowerCase() === "path";
  ...
  const pointsRaw = apartmentData.map(function (d, i) {
  ...
  function initiateZoom() {
    minZoom =
      Math.max($("#map-svg").width() / width, $("#map-svg").height() / height) *
      1;
    maxZoom = 20 * minZoom;
  ...
  function updateTooltip(event, d) {
    ...
    map_tooltip
      .html(
        `<strong>${d.statename || d.city || d.name}</strong><br/>
          Price: ${d.price.toFixed(2)}<br/>
          Bedrooms: ${d.bedrooms || "N/A"}<br/>
          Bathrooms: ${d.bathrooms || "N/A"} <br/>
          Pets Allowed: ${d.pets_allowed || "N/A"}
`
      )
      .style("left", xPos + "px")
      .style("top", yPos + "px");
  ...
  function displayPoints(filteredData, transform) {
    ...
    g.selectAll(".apartment-point")
      .data(filteredData)
      .enter()
      .append("path")
      .attr("class", "apartment-point")
      .attr("d", pinPath)
      .attr("transform", (d) => {
        const coords = projection([d.longitude, d.latitude]);
        let scale = 0.8 / (transform.k / 2); // change this value as needed
        let translateXOffset = scale * 11.67; // adjust this value as needed
        let translateYOffset = scale * 21.67; // adjust this value as needed

        return `translate(${coords[0] - translateXOffset}, ${
          coords[1] - translateYOffset
        }) scale(${scale})`;
      })
      ...
      .on("mouseout", function (event, d) {
        ...
        // Clear apartment info on mouseout
        d3.select("#info").html("");
      })
      .on("mousemove", function (event, d) {
      ...
      .on("click", function (event, d) {
        ...
        // Handle click to select apartment (if needed)
      });
  }
  ...
  function displayPointsInView(transform) {
    ...
    const filteredData = apartmentData.filter((d) => {
  ...
  function displayNodesInView(transform, clusterPoints) {
    ...
    const [x0, y0] = transform.invert([0, 0]);
    const [x1, y1] = transform.invert([width, height]);
    const filteredData = clusterPoints.filter((d) => {
      const [x, y, length] = [d[0], d[1], d[2].length];
      return x >= x0 && x <= x1 && y >= y0 && y <= y1 && length > 1;
    });
    const pinPointData = clusterPoints
      .filter((d) => {
        const [x, y, length] = [d[0], d[1], d[2].length];
        return x >= x0 && x <= x1 && y >= y0 && y <= y1 && length == 1;
      })
      .map((d) => apartmentData[d[2][0][2]]);
    displayPoints(pinPointData, transform);
    var pointSizeScale = d3
      .scaleLinear()
      .domain([
        d3.min(clusterPoints, function (d) {
          return d[2].length;
        }),
    ...
    g.selectAll(".centerPointText")
      .data(filteredData)
      .enter()
      .append("text")
      .attr("class", "centerPointText")
      .attr("x", function (d) {
        return d[0];
      })
      .attr("y", function (d) {
    ...
    // Convert filtered data to quadtree format
  }
  ...
  function handleStateClick(d) {
    ...
    data = states.find((c) => c.properties.name == d);
    boxZoom(path.bounds(data), path.centroid(data), 20);
  }
  function zoomToCoordinates(current, scale) {
    const target = projection([current.longitude, current.latitude]);
    const translate = [
  ...
  function zoomToAverageLocation(listings, scale) {
    ...
    const target = projection([avgLongitude, avgLatitude]);
    const translate = [
      width / 2 - target[0] * scale,
      height / 2 - target[1] * scale,
    ];
  ...
  const legendHeight = 64; // Increased to accommodate title
  const legendMargin = { right: 16, bottom: 16 };
  const legendPadding = 4; // Padding inside the legend background
  const titlePadding = 12; // Space between title and color scale
  ...
  legendG
    ...
    .attr("text-anchor", "middle")
    .attr("transform", `translate(${legendWidth / 2}, ${titlePadding + 5})`)
    .style("font-weight", "bold")
    .style("font-size", "12.8px")
    .text("Median Population");
  ...
  legendG
    .append("g")
    .attr("transform", `translate(0, ${titlePadding + 15})`)
    .call(legend);
  d3.selectAll(".label").style("font-size", "13px");
  var state_tooltip = d3
    .select("#map")
    .append("div")
    .style("opacity", 0)
    .attr("class", "tooltip")
  ...
  function updateTooltip2(event, d) {
    var xPos = event.pageX -150; // offset from mouse
    var yPos = event.pageY; // offset from mouse
    var stateData = groupedData.find(
      (state) => state.statename === d.properties.name
    );
    state_tooltip
      .html(
        `<strong>${d.properties.name}</strong><br/>
            Median Population: ${
              stateData ? stateData.population.toLocaleString() : "N/A"
            }
          `
      )
      .style("left", xPos + "px")
  ...
  g.append("g")
    ...
    // .attr("fill", "#e9d8a6")
    .attr("fill", (d) => {
      const stateData = groupedData.find(
        (state) => state.statename === d.properties.name
      );
    ...
    .on("mouseout", function (event, d) {
      ...
      d3.select(this).attr(
        "fill",
        stateData ? colorScale(stateData.population) : "#e9d8a6"
      );
    })
    .on("mousemove", function (event, d) {
...
function initializePricePerSqftChart(input) {
  const data = input
    .map((stateData) => ({
      ...
      cities: stateData.cities
        .map((cityData) => ({
          ...cityData,
          listings: [...cityData.listings].sort((a, b) => b.median - a.median),
        }))
  ...
  const width = $("#price-per-sqft").width() - 100;
  const height = $("#price-per-sqft").height();
  var main_margin = { top: 60, right: 10, bottom: 30, left: 100 },
    main_width = width - main_margin.left - main_margin.right,
    main_height = height - main_margin.top - main_margin.bottom;

  var mini_margin = { top: 60, right: 10, bottom: 30, left: 10 },
    mini_height = height - mini_margin.top - mini_margin.bottom,
    mini_width = 100 - mini_margin.left - mini_margin.right;
  function chartTitle() {
    if (currentLevel === "listing") {
      return `Living in <strong>${currentCity}</strong>, <strong>${currentState}</strong>`;
    } else if (currentLevel === "city") {
      return `Living in Cities of <strong>${currentState}</strong>`;
  ...
  svg
    .append("text")
    .attr("x", (main_width + mini_width) / 2 + main_margin.left)
    .attr("y", main_margin.top / 2)
    .attr("id", "price-title") // Corrected to use .attr for setting the class
    .attr("text-anchor", "middle")
    .attr("font-size", "24px")
    .text("Price Per Square Feet Chart");
  ...
  var miniGroup = svg
    ...
    .attr(
      "transform",
      `translate(${
        main_margin.left + main_width + main_margin.right + mini_margin.left
      },${mini_margin.top})`
    );
  ...
  var main_xAxis = d3.axisBottom(main_xScale).ticks(4).tickSize(0);

  //Add group for the x axis
  d3.select(".mainGroupWrapper")
    .append("g")
    .attr("class", "price-x-axis")
    .attr("transform", `translate(0,${main_height + 5})`);

  //Create y axis object
  var main_yAxis = d3.axisLeft(main_yScale).tickSize(0);

  //Add group for the y axis
  mainGroup
    .append("g")
    .attr("class", "price-y-axis")
  ...
  gBrush
    ...
    .attr("transform", (d, i) =>
      i
        ? `translate(${mini_width / 2},4) rotate(180)`
        : `translate(${mini_width / 2},-4) rotate(0)`
    );

  gBrush.selectAll("rect").attr("width", mini_width);

  //On a click recenter the brush window
  gBrush
    .select(".overlay")
    .on("mousedown.brush touchstart.brush", brushcenter)
    .on("touchmove", (event) => event.preventDefault())
    .on("touchend.brush", brushcenter);
  ...
  function updateScales(data) {
    main_xScale.domain([0, d3.max(data, (d) => d.median)]);
    mini_xScale.domain([0, d3.max(data, (d) => d.median)]);
    //   main_yScale.domain(data.map((d) => d.state));
    //   mini_yScale.domain(data.map((d) => d.state));
    main_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    mini_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    d3.select(".mainGroup").select(".price-y-axis").call(main_yAxis);
    d3.select("#dashboard-main-title")
      .transition()
      .duration(500)
      .style("opacity", 0)
      .on("end", function () {
    ...
    if (data.length > 11) {
      ...
      calcExtent = [
        mini_yScale(data[0].statename || data[0].city || data[0].name),
        mini_yScale(
          data[brushExtent].statename ||
            data[brushExtent].city ||
            data[brushExtent].name
        ),
      ];
    } else {
      calcExtent = [
    ...
    //UDPATE
    mini_bar
      .attr("width", (d) => mini_xScale(d.median))
      .attr("y", (d) => mini_yScale(d.statename || d.city || d.name))
  ...
  function updateTooltip(event, d) {
    ...
    price_tooltip
      .html(
        `<strong>${d.statename || d.city || d.name}</strong><br/>
        Price/Sqft Index: ${d.median.toFixed(2)}`
      )
      .style("left", xPos + "px")
      .style("top", yPos + "px");
  }
  function update(data) {
    ...
    bar
      .transition()
      .duration(100)
      .attr("y", (d) => main_yScale(d.statename || d.city || d.name))
      .attr("height", main_yScale.bandwidth())
      .attr("x", 0)
      .duration(50)
      .attr("width", (d) => main_xScale(d.median));
    //ENTER
    bar
      .enter()
      .append("rect")
      ...
      .on("mouseout", function (d) {
        ...
        d3.select(this).style("fill", function (d) {
          return this.getAttribute("selected-on") === "true"
            ? priceColor3
            : priceColor1;
        });
  ...
  function brushmove(event, data) {
    ...
    var selected = mini_yScale
      .domain()
      .filter(
        (d) =>
          extent[0] - mini_yScale.bandwidth() + 1e-2 <= mini_yScale(d) &&
          mini_yScale(d) <= extent[1] - 1e-2
      );
    //Update the colors of the mini chart - Make everything outside the brush grey
    d3.select(".miniGroup")
      .selectAll(".bar")
      .style("fill", (d) =>
        selected.includes(d.statename || d.city || d.name)
          ? priceColor1
          : "#e0e0e0"
      );

    //Update the label size
    d3.selectAll(".price-y-axis text").style(
      "font-size",
      textScale(selected.length)
    );
    ...
    var newMaxXScale = d3.max(data, (d) =>
      selected.includes(d.statename || d.city || d.name) ? d.median : 0
    );
  ...
  function brushcenter(event) {
    ...
    var center = Math.max(
      y0,
      Math.min(y1, d3.pointers(event, gBrush.node())[0][1])
    );
  ...
  function createGradient(idName, endPerc) {
    ...
    defs
      ...
      .enter()
      .append("stop")
      .attr("offset", (d, i) => i / (coloursRainbow.length - 1))
      .attr("stop-color", (d) => d);
  } //createGradient
...
function initializeAvgCrime(input) {
  ...
  const width = $("#average-crimes").width() - 200;
  const height = $("#average-crimes").height();
  var main_margin = { top: 60, right: 10, bottom: 20, left: 200 },
    main_width = width - main_margin.left - main_margin.right,
    main_height = height - main_margin.top - main_margin.bottom;

  var mini_margin = { top: 60, right: 0, bottom: 20, left: 10 },
    mini_height = height - mini_margin.top - mini_margin.bottom,
    mini_width = 100 - mini_margin.left - mini_margin.right;

  svg = d3
    .select("#average-crimes")
    .append("svg")
    .attr("class", "crimeSvgWrapper")
    .attr(
      "width",
      main_width +
        main_margin.left +
  ...
  svg
    ...
    .attr("x", (main_width + mini_width) / 2 + main_margin.left)
    .attr("y", main_margin.top / 2)
    .attr("id", "crime-title") // Corrected to use .attr for setting the class
    .attr("text-anchor", "middle")
    .attr("font-size", "24px")
    .text("Average Crime Chart");

  var mainGroup = svg
    .append("g")
    .attr("class", "crimeMainGroupWrapper")
    .attr("transform", `translate(${main_margin.left},${main_margin.top})`)
    .append("g")
    .attr("clip-path", "url(#clip)")
    .style("clip-path", "url(#clip)")
    .attr("class", "crimeMainGroup");
  mainGroup
    .append("rect")
    .attr("class", "crime-background-rect")
    .attr("width", main_width)
    .attr("height", main_height)
    .style("fill", "white")
    .style("pointer-events", "all")
  ...
  //Add group for the x axis
  d3.select(".crimeMainGroupWrapper")
    .append("g")
    .attr("class", "crime-x-axis")
    .attr("transform", `translate(0,${main_height + 5})`);
  ...
  var gCrimeBrush = d3
    .select(".crimeBrushGroup")
    .append("g")
    .attr("class", "crimeBrush")
    .call(crimeBrush);
  ...
  gCrimeBrush.selectAll("rect").attr("width", mini_width);

  //On a click recenter the brush window
  gCrimeBrush
    .select(".overlay")
    .on("mousedown.brush touchstart.brush", crimeBrushcenter)
    .on("touchmove", (event) => event.preventDefault())
    .on("touchend.brush", crimeBrushcenter);
  //Add the clip path for the main bar chart
  defs
    .append("clipPath")
    .attr("id", "clip")
  ...
  var calcExtent;
  function updateScales(data) {
    main_xScale.domain([0, d3.max(data, (d) => d.avg_crime)]);
    mini_xScale.domain([0, d3.max(data, (d) => d.avg_crime)]);
    //   main_yScale.domain(data.map((d) => d.state));
    //   mini_yScale.domain(data.map((d) => d.state));
    main_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    mini_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    ...
    //DATA JOIN
    var mini_bar = d3
      .select(".crimeMiniGroup")
      .selectAll(".bar")
      .data(data, (d, i) => i);

    //UDPATE
    mini_bar
      .attr("width", (d) => mini_xScale(d.avg_crime))
      .attr("y", (d) => mini_yScale(d.statename || d.city || d.name))
      .attr("height", mini_yScale.bandwidth());
  ...
  function updateTooltip(event, d) {
    ...
    var yPos = event.pageY - 70; // offset from mouse
    var { radarData, maxCrimeValue } = prepareRadarData(d);
    ...
    crime_tooltip.select(".radar-chart").html("");

    // Append the text information to the radar-chart div
    crime_tooltip
      .select(".radar-chart")
      .append("div")
      .html(
        `<strong>${d.statename || d.city || d.name}</strong><br/>
        Crime Index: ${d.avg_crime.toFixed(2)}`
      )
      .style("text-align", "center")
      .style("margin-bottom", "10px");

    RadarChart(".radar-chart", radarData, radarChartOptions);
    ...
    var tooltipHeight = tooltipNode.offsetHeight + 40;

    // Get the viewport dimensions
    var windowWidth = window.innerWidth;
    var windowHeight = window.innerHeight;

    // Adjust the tooltip position if it exceeds the viewport boundaries
  ...
  function prepareRadarData(d) {
    const radarData = [
      [
        { axis: "Violent Crime", value: d.violent_crime },
        // { axis: "Arson", value: d.arson_crime },
        { axis: "Assault", value: d.assault_crime },
        { axis: "Auto Theft", value: d.gta_crime },
        { axis: "Burglary", value: d.burg_crime },
        // { axis: "Larceny", value: d.larc_crime },
        // { axis: "Murder", value: d.murder_crime },
        // { axis: "Non-violent", value: d.nonviolent_crime },
        { axis: "Rape", value: d.rape_crime },
        { axis: "Robbery", value: d.robbery_crime },
      ],
    ];

    const maxCrimeValue = d.max_crime;
    return { radarData, maxCrimeValue };
  }
  function update(data) {
    /////////////////////////////////////////////////////////////
    ////////// Update the bars of the main bar chart ////////////
    ...
    bar
      ...
      .transition()
      .duration(50)
      .attr("width", (d) => main_xScale(d.avg_crime));
    //ENTER
    bar
      .enter()
      .append("rect")
      ...
      .on("mouseover", function (d) {
        ...
        d3.selectAll(".radar-chart").style("opacity", 1);

        // d3.selectAll("#crime-tooltip")
        // .style("width", "320px")
        // .style("height", "360px");
      })
      .on("mouseout", function (d) {
  ...
  function crimeBrushmove(event, data) {
    var extent = event.selection;
    crimeBrushSelection = event.selection;
    ...
    //Update the label size
    d3.selectAll(".crime-y-axis text").style(
      "font-size",
      textScale(selected.length)
    );
    ...
    //Find the new max of the bars to update the x scale
    var newMaxXScale = d3.max(data, (d) =>
      selected.includes(d.statename || d.city || d.name) ? d.avg_crime : 0
    );
    main_xScale.domain([0, newMaxXScale]);
  ...
  function scroll(event) {
    ...
    gCrimeBrush
      .call(crimeBrush.move, [topSection, topSection + size])
      .call(crimeBrush);
  }
  function createGradient(idName, endPerc) {
    var coloursRainbow = [
...
function initializeSumPet(input) {
    ...
    .map((stateData) => ({
      ...
      cities: stateData.cities
        .map((cityData) => ({
          ...cityData,
          listings: [...cityData.listings].sort(
            (a, b) => b.medFamInc - a.medFamInc
          ),
        }))
        .sort((a, b) => b.medFamInc - a.medFamInc),
    }))
    .sort((a, b) => b.medFamInc - a.medFamInc);
  ...
  svg
    ...
    .attr("x", (main_width + mini_width) / 2 + main_margin.left)
    .attr("y", main_margin.top / 2)
    .attr("id", "pet-title") // Corrected to use .attr for setting the class
    .attr("text-anchor", "middle")
    .attr("font-size", "24px")
    .text("Median Family Income Chart");
  var mainGroup = svg
    .append("g")
    .attr("class", "petMainGroupWrapper")
    .attr("transform", `translate(${main_margin.left},${main_margin.top})`)
    .append("g")
    .attr("clip-path", "url(#clip)")
    .style("clip-path", "url(#clip)")
    .attr("class", "petMainGroup");
  mainGroup
    .append("rect")
    .attr("class", "pet-background-rect")
    .attr("width", main_width)
    .attr("height", main_height)
    .style("fill", "white")
    .style("pointer-events", "all")
  ...
  //Create x axis object
  main_xAxis = d3.axisBottom(main_xScale).ticks(4).tickSize(0);

  //Add group for the x axis
  d3.select(".petMainGroupWrapper")
    .append("g")
    .attr("class", "pet-x-axis")
    .attr("transform", `translate(0,${main_height + 5})`);

  //Create y axis object
  main_yAxis = d3.axisLeft(main_yScale).tickSize(0);

  //Add group for the y axis
  mainGroup
    .append("g")
    .attr("class", "pet-y-axis")
    .attr("transform", "translate(-5,0)");
  ...
  var calcExtent;
  function updateScales(data) {
    main_xScale.domain([0, d3.max(data, (d) => d.medFamInc)]);
    mini_xScale.domain([0, d3.max(data, (d) => d.medFamInc)]);
    //   main_yScale.domain(data.map((d) => d.state));
    //   mini_yScale.domain(data.map((d) => d.state));
    main_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    mini_yScale.domain(data.map((d) => d.statename || d.city || d.name));
    ...
    //UDPATE
    mini_bar
      .attr("width", (d) => mini_xScale(d.medFamInc))
      .attr("y", (d) => mini_yScale(d.statename || d.city || d.name))
      .attr("height", mini_yScale.bandwidth());
  ...
  function updateTooltip(event, d) {
    ...
    pet_tooltip
      .html(
        ...
        Median Family Income Index: ${d.medFamInc.toFixed(2)}`
      )
      .style("left", xPos + "px")
      .style("top", yPos + "px");
  }
  function update(data) {
    ...
    bar
      ...
      .transition()
      .duration(50)
      .attr("width", (d) => main_xScale(d.medFamInc));
    //ENTER
    bar
      .enter()
      .append("rect")
      ...
      .on("mouseover", function (d) {
        d3.select(this).style("cursor", "pointer");
        d3.select(this).style("fill", socialColor2);
        pet_tooltip.style("opacity", 1);
        // pet_tooltip.style("width", "");
  ...
  function brushmove(event, data) {
    ...
    //Update the label size
    d3.selectAll(".pet-y-axis text").style(
      "font-size",
      textScale(selected.length)
    );
    ...
    //Find the new max of the bars to update the x scale
    var newMaxXScale = d3.max(data, (d) =>
      selected.includes(d.statename || d.city || d.name) ? d.medFamInc : 0
    );
    main_xScale.domain([0, newMaxXScale]);
